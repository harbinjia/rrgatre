...
def evaluate(model, data_iterator, params, mark='Val'):
    ...
    for batch in tqdm(data_iterator, unit='Batch', ascii=True):
        ...
        input_ids, attention_mask, edge_matrix, triples, input_tokens = batch
        bs, seq_len = input_ids.size()

        # inference
        with torch.no_grad():
            pred_seqs, pre_corres, xi, pred_rels = model(input_ids, attention_mask=attention_mask, edge_matrix=edge_matrix)
...
if __name__ == '__main__':
    ...
    _, predictions, ground_truths = evaluate(model, loader, params, mark=mode)
