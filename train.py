...
def train(model, data_iterator, optimizer, params):
    ...
    for step, _ in enumerate(t):
        ...
        input_ids, attention_mask, edge_matrix, seq_tags, potential_rels, rel_tags, subs, objs = batch

        # compute model output and loss
        loss, loss_seq, loss_mat, loss_rel = model(input_ids, attention_mask=attention_mask, edge_matrix=edge_matrix, seq_tags=seq_tags,
                                                   potential_rels=potential_rels, rel_tags=rel_tags,
                                                   subs=subs, objs=objs)
...
def train_and_evaluate(model, params, restore_file=None):
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        val_metrics, _, _ = evaluate(model, val_loader, params, mark='Val')
